...
char** loadLevel(const string& fileName, int& maxRow, int& maxCol, Player& player) {
    ...
    if (static_cast<double>(maxRow)*(maxCol) > static_cast<double>(INT32_MAX)){
        //cout << "You will have more than INT32_MAX tiles!" << endl;
        return nullptr;
    }
    ...
    //Check if there is extra data in the map file. If there is return null ptr
    inputFile >> currentChar;
    if (!inputFile.fail()){
        for (int row = 0; row < maxRow; row++){
            delete [] map[row];
    ...
    if (invalidChar || noExit){ //Was originally just invalidChar
        for (int row = 0; row < maxRow; row++){
            delete [] map[row];
        }
        delete [] map;
...
char** resizeMap(char** map, int& maxRow, int& maxCol) {
    ...
    //Make sure that we have valid dimmensions
    if (maxRow <= 0 || maxCol <= 0){
        return nullptr;
    }

    // You should probably do something about int
...
int doPlayerMove(char** map, int maxRow, int maxCol, Player& player, int nextRow, int nextCol) {
    if (nextRow < 0){
        //cout << "NextRow is negative!" << endl;
        return STATUS_STAY;
    }

    if (nextCol < 0){
        //cout << "NextCol is negative!" << endl;
        return STATUS_STAY;
    }

    if (nextRow >= maxRow){
        //cout << "NextRow is positive" << endl;
        return STATUS_STAY;
    }
    ...
    if (map[nextRow][nextCol] == '+'){
        //cout << "There is a pillar there!" << endl;
        return STATUS_STAY;
    }
    ...
    if (map[nextRow][nextCol] == '!'){
        if (player.treasure > 0){
            ...
            //cout << "Player has no treasure, so he cannot exit!" << endl;
            return STATUS_STAY;
        }
    }
    if (map[nextRow][nextCol] == '$'){
...
bool doMonsterAttack(char** map, int maxRow, int maxCol, const Player& player) {
    // Let's check the rows above the player first
    // We are going up, so we go from the tile above the player (player.row-1)
    // Then we make our way up to row 0 by subtracting row -1
    for (int row = player.row-1; row >= 0; row--){
        if (map[row][player.col] == '+'){
            break;
        }
        if (map[row][player.col] == 'M'){
            map[row][player.col] = '-';
    ...
    for (int col = player.col-1; col >= 0; col--){
        if (map[player.row][col] == '+'){
            break;
        }
        if (map[player.row][col] == 'M'){
            map[player.row][col] = '-';
